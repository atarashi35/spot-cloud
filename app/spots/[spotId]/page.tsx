...
import { getSpotMetadataById } from "@/lib/server/spots";
...
export async function generateMetadata({
  ...
  const spot = await getSpotMetadataById(spotId).catch(() => null);
  if (!spot) return { title: "SPOT" };

  const title = spot.name;
  const description = spot.shortDescription || spot.description.slice(0, 100);
  const image = spot.coverImageUrl && !isSvgAssetUrl(spot.coverImageUrl)
    ? spot.coverImageUrl
    : "/og-image.png";
