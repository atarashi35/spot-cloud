...
import { BookingListPanel } from "@/components/owner/booking-list-panel";
import { Breadcrumb } from "@/components/ui/breadcrumb";
import { useAuth } from "@/components/providers/auth-provider";
import { getSpotFromFirestore } from "@/lib/firestore/spots";

export function BookingManageClient({ spotId }: { spotId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-ink/68">出演依頼</p>
      </div>

      <BookingListPanel spotId={spotId} />
    </div>
  );
}
